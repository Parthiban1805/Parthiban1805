# Hi there! 👋 I'm Parthiban S

<div align="center">
  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/parthiban18)
[![Portfolio](https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=todoist&logoColor=white)](https://parthi-port.vercel.app)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:parthi.s1805@gmail.com)

</div>

## 🚀 About Me

**AI/ML Engineer** | **Full-Stack Developer** | **Computer Vision Specialist**

Currently pursuing B.Tech in **Artificial Intelligence and Data Science** at Bannari Amman Institute of Technology, specializing in building scalable, production-ready AI applications.

- 🔭 **AI/ML Engineer (Computer Vision)** at CubeAI Solutions Tech
- 💡 Delivering production-grade solutions with **YOLO**, **OpenCV**, and **MERN Stack**
- 🎯 Achieved **70% reduction** in manual monitoring time through AI automation
- 🏆 **Rajya Puraskar Awardee** for outstanding achievement in Scouts

---

## 🎯 Featured Projects

> **📌 Important Note:** Most of my projects are in **private repositories** as they are currently in **production** or under active development for clients. The public repositories showcase select work and experiments.

### 🔒 Current Production Work
- Industrial object detection and measurement systems
- Real-time AI inference pipelines (≤1 second processing)
- Full-stack AI applications with Flask, React, and MongoDB
- Computer vision solutions for industrial automation

### 🌟 What I'm Building
- 🤖 Production-ready AI solutions for real-world applications
- 🚀 Scalable microservices architectures
- 📚 LLM integration and RAG systems
- 🦀 High-performance computing with Rust

---

## 💻 Tech Stack

<div align="center">

### Languages
<img src="https://skillicons.dev/icons?i=python,javascript,java,c,cpp" />

### AI/ML & Computer Vision
<img src="https://skillicons.dev/icons?i=tensorflow,pytorch,opencv" />
**+ YOLO, Ultralytics, scikit-learn**

### Full-Stack Development
<img src="https://skillicons.dev/icons?i=react,nodejs,express,flask,mongodb,mysql" />

### Frontend & Styling
<img src="https://skillicons.dev/icons?i=html,css,tailwind,flutter" />

### DevOps & Tools
<img src="https://skillicons.dev/icons?i=docker,git,linux,vscode,github" />

</div>

---

## 🏢 Professional Experience

### 🤖 AI/ML Engineer - Computer Vision | CubeAI Solutions Tech
**May 2025 - Present**

- Developed AI-powered object detection and measurement systems for industrial automation
- Reduced manual monitoring time by **70%** through intelligent automation
- Built and deployed full-stack applications integrating computer vision models
- Achieved real-time processing performance (**≤1 second per inference**)
- Tech Stack: **YOLO, OpenCV, Flask, React, MongoDB**

## 📫 Let's Connect!

I'm always open to collaborating on innovative projects, discussing AI/ML technologies, or exploring new opportunities!

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/parthiban18)
[![Portfolio](https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=todoist&logoColor=white)](https://parthi-port.vercel.app)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:parthi.s1805@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Parthiban1805)

</div>

---

<div align="center">

⭐ **If you find my work interesting, feel free to reach out or star my repositories!**

*"Building intelligent systems that solve real-world problems"*

</div>
